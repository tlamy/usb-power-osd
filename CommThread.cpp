...
bool CommThread::ble_measure_loop(const SelectedDevice &device) {
  if (device.type != DeviceType::BLE) {
    std::cerr << "Wrong device type" << std::endl;
    return false;
  }

  auto target_peripheral = device.blePeripheral; // Copy the peripheral
  ...
  // (the reading and processing part stays the same)
}
...
void CommThread::setDevice(SelectedDevice *device) { this->device = device; }
