...
void OsdSettings::saveSettings() const {
  ...
  if (!config.HasEntry("always_on_top") ||
      config.ReadBool("always_on_top", always_on_top) != always_on_top) {
    config.Write("always_on_top", always_on_top);
  }
  if (!config.HasEntry("is_line_graph") ||
      config.ReadBool("is_line_graph", is_line_graph) != always_on_top) {
    config.Write("is_line_graph", is_line_graph);
  }
  int temp_int;
  wxString temp_string;
  if (!config.HasEntry("window_height") ||
      !config.Read("window_height", &temp_int) || temp_int != window_height) {
    config.Write("window_height", window_height);
  }
  if (!config.HasEntry("window_width") ||
      !config.Read("window_width", &temp_int) || temp_int != window_width) {
    config.Write("window_width", window_width);
  }
  if (!config.HasEntry("volts_amps_font") ||
      !config.Read("volts_amps_font", &temp_string) ||
      temp_string != volts_amps_font) {
    config.Write("volts_amps_font", wxString(volts_amps_font));
  }
  if (!config.HasEntry("volts_font_size") ||
      !config.Read("volts_font_size", &temp_int) ||
      temp_int != volts_font_size) {
    config.Write("volts_font_size", volts_font_size);
  }
  if (!config.HasEntry("amps_font_size") ||
      !config.Read("amps_font_size", &temp_int) || temp_int != amps_font_size) {
    config.Write("amps_font_size", amps_font_size);
  }
  if (!config.HasEntry("graph_height") ||
      !config.Read("graph_height", &temp_int) || temp_int != graph_height) {
    config.Write("graph_height", graph_height);
  }
  if (!config.HasEntry("color_amps") ||
      !config.Read("color_amps", &temp_string) ||
      temp_string != rgb_to_string(color_amps)) {
    config.Write("color_amps", rgb_to_string(color_amps));
  }
  if (!config.HasEntry("color_5v") || !config.Read("color_5v", &temp_string) ||
      temp_string != rgb_to_string(color_5v)) {
    config.Write("color_5v", rgb_to_string(color_5v));
  }
  if (!config.HasEntry("color_9v") || !config.Read("color_9v", &temp_string) ||
      temp_string != rgb_to_string(color_9v)) {
    config.Write("color_9v", rgb_to_string(color_9v));
  }
  if (!config.HasEntry("color_15v") ||
      !config.Read("color_15v", &temp_string) ||
      temp_string != rgb_to_string(color_15v)) {
    config.Write("color_15v", rgb_to_string(color_15v));
  }
  if (!config.HasEntry("color_20v") ||
      !config.Read("color_20v", &temp_string) ||
      temp_string != rgb_to_string(color_20v)) {
    config.Write("color_20v", rgb_to_string(color_20v));
  }
  if (!config.HasEntry("color_28v") ||
      !config.Read("color_28v", &temp_string) ||
      temp_string != rgb_to_string(color_28v)) {
    config.Write("color_28v", rgb_to_string(color_28v));
  }
  if (!config.HasEntry("color_36v") ||
      !config.Read("color_36v", &temp_string) ||
      temp_string != rgb_to_string(color_36v)) {
    config.Write("color_36v", rgb_to_string(color_36v));
  }
  if (!config.HasEntry("color_48v") ||
      !config.Read("color_48v", &temp_string) ||
      temp_string != rgb_to_string(color_48v)) {
    config.Write("color_48v", rgb_to_string(color_48v));
  }
  config.Flush();
}

wxColour OsdSettings::color_setting(const wxFileConfig *config,
                                    const wxString &key,
...
void OsdSettings::loadSettings() {
  ...
  if (config->HasEntry("window_height"))
    window_height =
        static_cast<int>(config->ReadLong("window_height", window_height));
  if (config->HasEntry("window_width"))
    window_width =
        static_cast<int>(config->ReadLong("window_width", window_width));
  if (config->HasEntry("volts_amps_font"))
    volts_amps_font = config->Read("volts_amps_font", volts_amps_font);
  if (config->HasEntry("volts_font_size"))
    volts_font_size =
        static_cast<int>(config->ReadLong("volts_font_size", volts_font_size));
  if (config->HasEntry("amps_font_size"))
    amps_font_size =
        static_cast<int>(config->ReadLong("amps_font_size", amps_font_size));
  if (config->HasEntry("graph_height"))
    graph_height =
        static_cast<int>(config->ReadLong("graph_height", graph_height));
  if (config->HasEntry("color_amps"))
...
wxColour OsdSettings::setting2Rgb(const wxString &setting) {
  ...
  return wxColour(wxAtoi(tokens.Item(0)), wxAtoi(tokens.Item(1)),
                  wxAtoi(tokens.Item(2)));
}
