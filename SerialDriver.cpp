...
ceSerial::ceSerial(const std::string &Device, long BaudRate, long DataSize,
                   char ParityType, float NStopBits)
    : stdbaud(true) {
#ifdef CE_WINDOWS
...
void ceSerial::SetPortName(const std::string &Device) { port = Device; }

std::string ceSerial::GetPort() { return port; }
...
long ceSerial::GetDataSize() { return dsize; }
...
char ceSerial::GetParity() { return parity; }
...
float ceSerial::GetStopBits() { return stopbits; }
...
long ceSerial::GetBaudRate() { return baud; }
...
#endif
  hComm = CreateFile(wtext.c_str(), GENERIC_READ | GENERIC_WRITE, 0, 0,
                     OPEN_EXISTING,
...
bool ceSerial::WriteChar(const char ch) { return Write((char *)&ch, 1); }
...
bool ceSerial::WriteChar(const char ch) { return Write(&ch, 1); }
...
wxSerialPort::wxSerialPort()
    : m_serialPort(m_io_context), m_timer(m_io_context) {
  // Default ctor
  Init();
...
wxSerialPort::wxSerialPort(const char *device)
    : m_serialPort(m_io_context, device), m_timer(m_io_context),
...
wxSerialPort::wxSerialPort(const wxSerialPort &other)
    ...
    : m_serialPort(m_io_context) /*, m_timer(other.m_timer)*/,
      m_timer(m_io_context), m_strPortName(other.m_strPortName) {
  // Copy ctor
  Init();
}
...
BOOST_ASIO_SYNC_OP_VOID wxSerialPort::Assign(
    const boost::asio::serial_port::native_handle_type &native_serial_port) {
...
std::size_t
wxSerialPort::DoReadSome(const boost::asio::mutable_buffer &buffer) {
  boost::system::error_code error;
...
std::size_t wxSerialPort::DoReadSome(const boost::asio::mutable_buffer &buffer,
  ...
  m_serialPort.async_read_some(
      buffer,
      /*std::bind(&wxSerialPort::AsyncReadHandler, this, std::placeholders::_1,
         std::placeholders::_2)*/
...
std::size_t wxSerialPort::WriteSome(const void *pszBuffer,
...
std::size_t wxSerialPort::DoWriteSome(const boost::asio::const_buffer &buffer,
  ...
  m_serialPort.async_write_some(
      buffer,
      /*std::bind(&wxSerialPort::AsyncWriteHandler, this, std::placeholders::_1,
         std::placeholders::_2)*/
      [&](const boost::system::error_code &error,
          std::size_t bytes_transferred) {
...
wxSerialPort::DoAsyncReadSome(const boost::asio::mutable_buffer &buffer,
...
BOOST_ASIO_INITFN_RESULT_TYPE(ReadHandler, void(boost::system::error_code,
                                                std::size_t)) // Return type
wxSerialPort::AsyncReadSome(std::string &strBuffer,
                            BOOST_ASIO_MOVE_ARG(ReadHandler) handler) {
...
BOOST_ASIO_INITFN_RESULT_TYPE(ReadHandler, void(boost::system::error_code,
                                                std::size_t)) // Return type
wxSerialPort::AsyncReadSome(wxCharBuffer &chBuffer,
                            BOOST_ASIO_MOVE_ARG(ReadHandler) handler) {
  return DoAsyncReadSome(
      boost::asio::buffer(chBuffer.data(), chBuffer.length()), handler);
}

template <typename ReadHandler>
BOOST_ASIO_INITFN_RESULT_TYPE(ReadHandler, void(boost::system::error_code,
                                                std::size_t)) // Return type
wxSerialPort::AsyncReadSome(void *pszBuffer, const std::size_t uiSize,
                            BOOST_ASIO_MOVE_ARG(ReadHandler) handler) {
...
BOOST_ASIO_INITFN_RESULT_TYPE(ReadHandler, void(boost::system::error_code,
                                                std::size_t)) // Return type
wxSerialPort::AsyncReadSome(std::vector<type> &vBuffer,
                            BOOST_ASIO_MOVE_ARG(ReadHandler) handler) {
...
BOOST_ASIO_INITFN_RESULT_TYPE(ReadHandler, void(boost::system::error_code,
                                                std::size_t)) // Return type
wxSerialPort::AsyncReadSome(std::array<type, uiSize> &arrBuffer,
                            BOOST_ASIO_MOVE_ARG(ReadHandler) handler) {
...
BOOST_ASIO_INITFN_RESULT_TYPE(WriteHandler, void(boost::system::error_code,
                                                 std::size_t)) // Return type
wxSerialPort::DoAsyncWriteSome(const boost::asio::const_buffer &buffer,
...
BOOST_ASIO_INITFN_RESULT_TYPE(WriteHandler, void(boost::system::error_code,
                                                 std::size_t)) // Return type
wxSerialPort::AsyncWriteSome(const std::string &strBuffer,
                             BOOST_ASIO_MOVE_ARG(WriteHandler) handler) {
...
BOOST_ASIO_INITFN_RESULT_TYPE(WriteHandler, void(boost::system::error_code,
                                                 std::size_t)) // Return type
wxSerialPort::AsyncWriteSome(const wxString &strBuffer,
                             BOOST_ASIO_MOVE_ARG(WriteHandler) handler) {
  return DoWriteSome(boost::asio::buffer(strBuffer.data(), strBuffer.size()),
                     handler);
}
...
BOOST_ASIO_INITFN_RESULT_TYPE(WriteHandler, void(boost::system::error_code,
                                                 std::size_t)) // Return type
wxSerialPort::AsyncWriteSome(const wxCharBuffer &chBuffer,
                             BOOST_ASIO_MOVE_ARG(WriteHandler) handler) {
  return DoWriteSome(boost::asio::buffer(chBuffer.data(), chBuffer.length()),
                     handler);
}
...
BOOST_ASIO_INITFN_RESULT_TYPE(WriteHandler, void(boost::system::error_code,
                                                 std::size_t)) // Return type
wxSerialPort::AsyncWriteSome(const void *pszBuffer, const std::size_t uiSize,
                             BOOST_ASIO_MOVE_ARG(WriteHandler) handler) {
...
BOOST_ASIO_INITFN_RESULT_TYPE(WriteHandler, void(boost::system::error_code,
                                                 std::size_t)) // Return type
wxSerialPort::AsyncWriteSome(const std::vector<type> &vBuffer,
                             BOOST_ASIO_MOVE_ARG(WriteHandler) handler) {
...
BOOST_ASIO_INITFN_RESULT_TYPE(WriteHandler, void(boost::system::error_code,
                                                 std::size_t)) // Return type
wxSerialPort::AsyncWriteSome(const std::array<type, uiSize> &arrBuffer,
                             BOOST_ASIO_MOVE_ARG(WriteHandler) handler) {
...
std::size_t wxSerialPort::Write(const void *pszBuffer,
                                const std::size_t uiSize) {
...
BOOST_ASIO_INITFN_RESULT_TYPE(ReadHandler, void(boost::system::error_code,
                                                std::size_t)) // Return type
wxSerialPort::DoAsyncRead(const boost::asio::mutable_buffer &buffer,
                          BOOST_ASIO_MOVE_ARG(ReadHandler) handler) {
...
BOOST_ASIO_INITFN_RESULT_TYPE(ReadHandler, void(boost::system::error_code,
                                                std::size_t)) // Return type
wxSerialPort::AsyncRead(std::string &strBuffer,
                        BOOST_ASIO_MOVE_ARG(ReadHandler) handler) {
...
BOOST_ASIO_INITFN_RESULT_TYPE(ReadHandler, void(boost::system::error_code,
                                                std::size_t)) // Return type
wxSerialPort::AsyncRead(wxCharBuffer &chBuffer,
                        BOOST_ASIO_MOVE_ARG(ReadHandler) handler) {
  return DoAsyncRead(boost::asio::buffer(chBuffer.data(), chBuffer.length()),
                     handler);
}
...
BOOST_ASIO_INITFN_RESULT_TYPE(ReadHandler, void(boost::system::error_code,
                                                std::size_t)) // Return type
wxSerialPort::AsyncRead(void *pszBuffer, const std::size_t uiSize,
                        BOOST_ASIO_MOVE_ARG(ReadHandler) handler) {
...
BOOST_ASIO_INITFN_RESULT_TYPE(ReadHandler, void(boost::system::error_code,
                                                std::size_t)) // Return type
wxSerialPort::AsyncRead(std::vector<type> &vBuffer,
                        BOOST_ASIO_MOVE_ARG(ReadHandler) handler) {
...
BOOST_ASIO_INITFN_RESULT_TYPE(ReadHandler, void(boost::system::error_code,
                                                std::size_t)) // Return type
wxSerialPort::AsyncRead(std::array<type, uiSize> &arrBuffer,
                        BOOST_ASIO_MOVE_ARG(ReadHandler) handler) {
...
BOOST_ASIO_INITFN_RESULT_TYPE(WriteHandler, void(boost::system::error_code,
                                                 std::size_t)) // Return type
wxSerialPort::DoAsyncWrite(const boost::asio::const_buffer &buffer,
                           BOOST_ASIO_MOVE_ARG(WriteHandler) handler) {
...
BOOST_ASIO_INITFN_RESULT_TYPE(WriteHandler, void(boost::system::error_code,
                                                 std::size_t)) // Return type
wxSerialPort::AsyncWrite(const std::string &strBuffer,
                         BOOST_ASIO_MOVE_ARG(WriteHandler) handler) {
...
BOOST_ASIO_INITFN_RESULT_TYPE(WriteHandler, void(boost::system::error_code,
                                                 std::size_t)) // Return type
wxSerialPort::AsyncWrite(const wxString &strBuffer,
                         BOOST_ASIO_MOVE_ARG(WriteHandler) handler) {
  return DoWrite(boost::asio::buffer(strBuffer.data(), strBuffer.size()),
                 handler);
}
...
BOOST_ASIO_INITFN_RESULT_TYPE(WriteHandler, void(boost::system::error_code,
                                                 std::size_t)) // Return type
wxSerialPort::AsyncWrite(const wxCharBuffer &chBuffer,
                         BOOST_ASIO_MOVE_ARG(WriteHandler) handler) {
  return DoWrite(boost::asio::buffer(chBuffer.data(), chBuffer.length()),
                 handler);
}
...
BOOST_ASIO_INITFN_RESULT_TYPE(WriteHandler, void(boost::system::error_code,
                                                 std::size_t)) // Return type
wxSerialPort::AsyncWrite(const void *pszBuffer, const std::size_t uiSize,
                         BOOST_ASIO_MOVE_ARG(WriteHandler) handler) {
...
BOOST_ASIO_INITFN_RESULT_TYPE(WriteHandler, void(boost::system::error_code,
                                                 std::size_t)) // Return type
wxSerialPort::AsyncWrite(const std::vector<type> &vBuffer,
                         BOOST_ASIO_MOVE_ARG(WriteHandler) handler) {
...
BOOST_ASIO_INITFN_RESULT_TYPE(WriteHandler, void(boost::system::error_code,
                                                 std::size_t)) // Return type
wxSerialPort::AsyncWrite(const std::array<type, uiSize> &arrBuffer,
                         BOOST_ASIO_MOVE_ARG(WriteHandler) handler) {
...
// This helper function is intended for internal use by the class itself
std::size_t wxSerialPort::DoReadUntil(const boost::asio::mutable_buffer &buffer,
                                      const boost::asio::const_buffer &delim) {
  boost::system::error_code error;
  std::size_t ret = boost::asio::read(
      m_serialPort, buffer,
      [&](const boost::system::error_code &error,
...
std::size_t wxSerialPort::ReadUntil(std::string &strBuffer,
                                    const std::string &strDelim) {
  return DoReadUntil(boost::asio::buffer(strBuffer),
                     boost::asio::buffer(strDelim));
}

std::size_t wxSerialPort::ReadUntil(wxCharBuffer &chBuffer,
                                    const wxCharBuffer &chDelim) {
  return DoReadUntil(boost::asio::buffer(chBuffer.data(), chBuffer.length()),
                     boost::asio::buffer(chDelim.data(), chDelim.length()));
}

std::size_t wxSerialPort::ReadUntil(void *pszBuffer, const std::size_t uiSize1,
                                    const void *pszDelim,
                                    const std::size_t uiSize2) {
  return DoReadUntil(boost::asio::buffer(pszBuffer, uiSize1),
                     boost::asio::buffer(pszDelim, uiSize2));
}

template <typename type>
std::size_t wxSerialPort::ReadUntil(std::vector<type> &vBuffer,
                                    const std::vector<type> &vDelim) {
...
std::size_t wxSerialPort::ReadUntil(std::array<type, uiSize1> &arrBuffer,
                                    const std::array<type, uiSize2> &arrDelim) {
  return DoReadUntil(boost::asio::buffer(arrBuffer),
...
std::size_t wxSerialPort::DoReadUntil(const boost::asio::mutable_buffer &buffer,
  ...
  boost::asio::async_read(
      ...
      /*std::bind(&wxSerialPort::AsyncReadUntilHandler, this,
...
std::size_t wxSerialPort::ReadUntil(std::string &strBuffer,
                                    ...
                                    const int timeout) {
  return DoReadUntil(boost::asio::buffer(strBuffer),
                     boost::asio::buffer(strDelim), timeout);
}
...
std::size_t wxSerialPort::ReadUntil(void *pszBuffer, const std::size_t uiSize1,
                                    ...
                                    const int timeout) {
  return DoReadUntil(boost::asio::buffer(pszBuffer, uiSize1),
                     boost::asio::buffer(pszDelim, uiSize2), timeout);
}
...
std::size_t wxSerialPort::ReadUntil(std::array<type, uiSize1> &arrBuffer,
                                    ...
                                    const int timeout) {
  return DoReadUntil(boost::asio::buffer(arrBuffer),
                     boost::asio::buffer(arrDelim), timeout);
}
...
BOOST_ASIO_INITFN_RESULT_TYPE(ReadUntilHandler,
                              void(boost::system::error_code,
                                   std::size_t)) // Return type
wxSerialPort::DoAsyncReadUntil(const boost::asio::mutable_buffer &buffer,
                               const boost::asio::const_buffer &delim,
                               BOOST_ASIO_MOVE_ARG(ReadUntilHandler) handler) {
...
BOOST_ASIO_INITFN_RESULT_TYPE(ReadUntilHandler,
                              void(boost::system::error_code,
                                   std::size_t)) // Return type
wxSerialPort::AsyncReadUntil(std::string &strBuffer,
                             const std::string &strDelim,
                             BOOST_ASIO_MOVE_ARG(ReadUntilHandler) handler) {
  return DoAsyncReadUntil(boost::asio::buffer(strBuffer),
                          boost::asio::buffer(strDelim), handler);
}
...
BOOST_ASIO_INITFN_RESULT_TYPE(ReadUntilHandler,
                              void(boost::system::error_code,
                                   std::size_t)) // Return type
wxSerialPort::AsyncReadUntil(wxCharBuffer &chBuffer,
                             const wxCharBuffer &chDelim,
                             BOOST_ASIO_MOVE_ARG(ReadUntilHandler) handler) {
  return DoAsyncReadUntil(
      boost::asio::buffer(chBuffer.data(), chBuffer.length()),
      boost::asio::buffer(chDelim.data(), chDelim.length()), handler);
}
...
BOOST_ASIO_INITFN_RESULT_TYPE(ReadUntilHandler,
                              void(boost::system::error_code,
                                   std::size_t)) // Return type
wxSerialPort::AsyncReadUntil(void *pszBuffer, const std::size_t uiSize1,
                             const void *pszDelim, const std::size_t uiSize2,
                             BOOST_ASIO_MOVE_ARG(ReadUntilHandler) handler) {
  return DoAsyncReadUntil(boost::asio::buffer(pszBuffer, uiSize1),
                          boost::asio::buffer(pszDelim, uiSize2), handler);
}
...
BOOST_ASIO_INITFN_RESULT_TYPE(ReadUntilHandler,
                              void(boost::system::error_code,
                                   std::size_t)) // Return type
wxSerialPort::AsyncReadUntil(std::vector<type> &vBuffer,
                             const std::vector<type> &vDelim,
                             BOOST_ASIO_MOVE_ARG(ReadUntilHandler) handler) {
  return DoAsyncReadUntil(boost::asio::buffer(vBuffer),
                          boost::asio::buffer(vDelim), handler);
}

template <typename type, std::size_t uiSize1, std::size_t uiSize2,
          typename ReadUntilHandler>
BOOST_ASIO_INITFN_RESULT_TYPE(ReadUntilHandler,
                              void(boost::system::error_code,
                                   std::size_t)) // Return type
wxSerialPort::AsyncReadUntil(std::array<type, uiSize1> &arrBuffer,
                             const std::array<type, uiSize2> &arrDelim,
                             BOOST_ASIO_MOVE_ARG(ReadUntilHandler) handler) {
  return DoAsyncReadUntil(boost::asio::buffer(arrBuffer),
                          boost::asio::buffer(arrDelim), handler);
}
...
wxSerialPort::AsyncReadUntil(std::string &strBuffer,
                             ...
                             const int timeout) {
  return DoAsyncReadUntil(boost::asio::buffer(strBuffer),
                          boost::asio::buffer(strDelim), handler, timeout);
}
...
wxSerialPort::AsyncReadUntil(wxCharBuffer &chBuffer,
  ...
  return DoAsyncReadUntil(
      boost::asio::buffer(chBuffer.data(), chBuffer.length()),
      boost::asio::buffer(chDelim.data(), chDelim.length()), handler, timeout);
}
...
wxSerialPort::AsyncReadUntil(std::vector<type> &vBuffer,
                             ...
                             const int timeout) {
  return DoAsyncReadUntil(boost::asio::buffer(vBuffer),
                          boost::asio::buffer(vDelim), handler, timeout);
}
...
wxSerialPort::AsyncReadUntil(std::array<type, uiSize1> &arrBuffer,
                             ...
                             const int timeout) {
  return DoAsyncReadUntil(boost::asio::buffer(arrBuffer),
                          boost::asio::buffer(arrDelim), handler, timeout);
}
...
// This helper function is intended for internal use by the class itself
std::size_t
wxSerialPort::DoCompletionCondition(const boost::asio::mutable_buffer &buffer,
                                    const boost::asio::const_buffer &delim,
                                    const boost::system::error_code &error,
                                    const std::size_t bytes_transferred) const {
  // Look for a match
  auto it = std::search(boost::asio::buffers_begin(buffer),
                        boost::asio::buffers_begin(buffer) + bytes_transferred,
                        boost::asio::buffers_begin(delim),
                        boost::asio::buffers_end(delim));

  return (!!error ||
          it != (boost::asio::buffers_begin(buffer) + bytes_transferred))
             ? 0
             : boost::asio::detail::default_max_transfer_size_t::
...
boost::asio::serial_port::executor_type
wxSerialPort::GetExecutor() BOOST_ASIO_NOEXCEPT {
...
BOOST_ASIO_SYNC_OP_VOID wxSerialPort::SetOption(const FlowControl option) {
  boost::asio::serial_port_base::flow_control flowcontrol(
      static_cast<boost::asio::serial_port_base::flow_control::type>(option));
...
BOOST_ASIO_SYNC_OP_VOID wxSerialPort::SetOption(const Parity option) {
  boost::asio::serial_port_base::parity parity(
      static_cast<boost::asio::serial_port_base::parity::type>(option));
...
BOOST_ASIO_SYNC_OP_VOID wxSerialPort::SetOption(const StopBits option) {
  boost::asio::serial_port_base::stop_bits stopbits(
      static_cast<boost::asio::serial_port_base::stop_bits::type>(option));
...
wxSerialPort::SetFlowControl(const FlowControl flowcontrol) {
...
bool wxSerialPort::IsOpen() const { return m_serialPort.is_open(); }
...
BOOST_ASIO_INITFN_RESULT_TYPE(WaitHandler,
                              void(boost::system::error_code)) // Return type
...
wxString wxSerialPort::GetPortName() const { return m_strPortName; }
...
BOOST_ASIO_SYNC_OP_VOID wxSerialPort::WaitForBuffers(const Buffers buffers,
                                                     const int timeout) {
  ...
  boost::asio::post(m_io_context, [&] {
    for (;;) {
      ...
      if (((buffers & buf_In) && nNumByInBuf) ||
          ((buffers & buf_Out) && nNumByOutBuf)) {
        m_io_context.stop();
        m_timer.cancel();
        return;
      }
...
// Async read handler
// void wxSerialPort::AsyncReadHandler(const boost::system::error_code& error,
// std::size_t bytes_transferred)
//{
//    // Read operation was not aborted
//    if (!error) // != boost::asio::error::operation_aborted
...
// Async write handler
// void wxSerialPort::AsyncWriteHandler(const boost::system::error_code& error,
// std::size_t bytes_transferred)
//{
//    // Write operation was not aborted
//    if (!error) // != boost::asio::error::operation_aborted
