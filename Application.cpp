...
bool MyApp::OnInit() {
  ...
  auto frame =
      new MainFrame("MacWake USB-C OSD", wxDefaultPosition, wxSize(400, 250));
  frame->Show(true);
  return true;
}
...
MainFrame::MainFrame(const wxString &title, const wxPoint &pos,
                     const wxSize &size)
    : wxFrame( // NOLINT(*-pro-type-member-init)
  ...
  this->m_graph_style =
      settings.is_line_graph ? GraphPanel::STYLE_LINE : GraphPanel::STYLE_BAR;
  this->m_alwaysontop = settings.always_on_top;

  auto *viewMenu = new wxMenu;
  viewMenu->AppendCheckItem(CHARGE_ENABLED, wxT("&Enable charge display"));
  viewMenu->AppendCheckItem(
      ALWAYSONTOP, wxT("&Always on top"),
      wxT("Keep window on top, even if app is in background"));
  viewMenu->AppendCheckItem(LINEGRAPH, wxT("&Line Graph style"),
                            wxT("Use line graph instead of bar graph"));
  viewMenu->Check(ALWAYSONTOP, settings.always_on_top);
  viewMenu->Check(CHARGE_ENABLED, settings.is_charge_enabled);
  ...
  viewMenu->Append(wxID_PREFERENCES, wxT("&Settings…\tCtrl+,"),
                   wxT("Open settings dialog"));
  ...
  Bind(wxEVT_MENU, &MainFrame::ToggleCharge, this, CHARGE_ENABLED);
  ...
  this->m_amphours =
      new wxStaticText(panel, wxID_ANY, "---", wxDefaultPosition,
                       wxSize(size.GetWidth() / 2, -1), wxALIGN_RIGHT);
  this->m_current_minmax = new wxStaticText(panel, wxID_ANY, "");

  // (Optional) You can also set font or style:
  wxFont font(fontSize, wxFONTFAMILY_TELETYPE, wxFONTSTYLE_NORMAL,
              wxFONTWEIGHT_NORMAL);
  wxFont statusFont(20, wxFONTFAMILY_SWISS, wxFONTSTYLE_NORMAL,
                    wxFONTWEIGHT_NORMAL);
  wxFont minMaxFont(fontSize / 2, wxFONTFAMILY_TELETYPE, wxFONTSTYLE_NORMAL,
                    wxFONTWEIGHT_NORMAL);
  ...
  this->m_status_text = new wxStaticText(
      panel, wxID_ANY, "Starting up", wxDefaultPosition,
  ...
  m_amphourSizer = new wxBoxSizer(wxHORIZONTAL);
  ...
  minMaxRowSizer->Add(this->m_current_minmax, 0,
  ...
  m_resetButton = new wxButton(panel, wxID_ANY, wxT("X"));
  ...
  m_resetButton->Show(settings.is_charge_enabled);
  m_amphours->Show(settings.is_charge_enabled);

  // Add widgets to sizer and tell sizer about their visibility
  amphourSizer->Add(m_resetButton, 0, 0, 5);
  amphourSizer->Show(m_resetButton, settings.is_charge_enabled);

  amphourSizer->AddStretchSpacer(1);

  amphourSizer->Add(this->m_amphours, 0, wxEXPAND | wxALL, 5);
  amphourSizer->Show(this->m_amphours, settings.is_charge_enabled);

  mainSizer->Add(amphourSizer, 0, wxEXPAND | wxALL, 0);
...
void MainFrame::Help(wxCommandEvent &event) { About(event); }
...
void MainFrame::ToggleCharge(wxCommandEvent &event) {
  settings.is_charge_enabled = !settings.is_charge_enabled;
  this->OnChargeSettingChanged(settings.is_charge_enabled);
}

void MainFrame::ToggleLineGraph(wxCommandEvent &event) {
  this->m_graph_style = this->m_graph_style == GraphPanel::STYLE_BAR
                            ? GraphPanel::STYLE_LINE
                            : GraphPanel::STYLE_BAR;
...
void MainFrame::OnGrapthTypeChanged(const bool isLineGraph) {
  this->m_graph_style =
      isLineGraph ? GraphPanel::STYLE_LINE : GraphPanel::STYLE_BAR;
  this->m_graph_panel->SetGraphStyle(this->m_graph_style);
  wxMenuItem *item = GetMenuBar()->FindItem(LINEGRAPH);
  if (item) {
    item->Check(m_graph_style == GraphPanel::STYLE_LINE);
...
void MainFrame::OnStatusUpdate(const wxThreadEvent &event) {
  if (const wxString &status = event.GetString(); status.empty()) {
    ...
    if (!this->m_show_status) {
      ...
      this->m_status_text->GetContainingSizer()->Show(this->m_status_text,
                                                      true);
      this->m_status_text->GetContainingSizer()->Layout();
      this->Refresh();
    }
...
void MainFrame::OnDataUpdate(wxThreadEvent &event) {
  ...
  const wxString min_str = wxString::Format(
      "%0.3fA-%0.3fA", minCurrent / 1000.0, maxCurrent / 1000.0);
  ...
  if (theEvent->GetMilliVolts() > 500 &&
    ...
    if (settings.is_charge_enabled) {
      auto now = wxMilliClockToLong(wxGetLocalTimeMillis());
      if (this->m_last_timestamp > 0 && (now > this->m_last_timestamp + 2000 ||
                                         now < this->m_last_timestamp)) {
        this->m_last_timestamp = now;
      }
      if (this->m_last_timestamp < now) {
        auto timeDiffMillis = now - m_last_timestamp;
        auto hours = static_cast<double>(timeDiffMillis) *
                     static_cast<double>(theEvent->GetMilliAmps()) / 3600000.0;
        this->m_milliamphours += hours;
        this->m_last_timestamp = now;
      }
      this->m_amphours->SetLabel(
          wxString::Format("%0.1fmAh", this->m_milliamphours));
    }
  }
}

void MainFrame::OnFontChanged(const wxFont &wx_font) {
  auto mainFont = new wxFont(wx_font.GetPointSize(), wx_font.GetFamily(),
                             wx_font.GetStyle(), wx_font.GetWeight());
  mainFont->SetFaceName(wx_font.GetFaceName());
  this->m_volts_font = mainFont;
  this->m_amps_font = mainFont;
  wxFont minMaxFont(wx_font.GetPointSize() / 2, wx_font.GetFamily(),
                    wx_font.GetStyle(), wx_font.GetWeight());
...
void MainFrame::OnChargeSettingChanged(const bool is_charge_enabled) {
  if (is_charge_enabled) {
    this->m_last_timestamp = 0;
    this->m_milliamphours = 0.0;
    this->m_amphours->Show();
    this->m_resetButton->Show();
    // Show the widgets in their sizer
    m_amphourSizer->Show(this->m_amphours, true);
    m_amphourSizer->Show(this->m_resetButton, true);

    // Change the sizer proportion so it takes up space
    wxSizer *mainSizer = this->GetSizer();
    if (mainSizer) {
      wxSizerItem *item = mainSizer->GetItem(m_amphourSizer);
      if (item) {
    ...
    this->m_amphours->Hide();
    this->m_resetButton->Hide();
    // Hide the widgets in their sizer
    m_amphourSizer->Show(this->m_amphours, false);
    m_amphourSizer->Show(this->m_resetButton, false);

    // Change the sizer proportion so it doesn't take up space
    wxSizer *mainSizer = this->GetSizer();
    if (mainSizer) {
      wxSizerItem *item = mainSizer->GetItem(m_amphourSizer);
      if (item) {
  ...
  wxMenuItem *item = GetMenuBar()->FindItem(CHARGE_ENABLED);
  if (item) {
    item->Check(is_charge_enabled);
  }
}

void MainFrame::About(wxCommandEvent &) {
  wxMessageBox(wxT("An On-Screen-Display for MacWake's  or PLDaniels's USB-C "
                   "Amp Meter\n\n") wxT("Author: Thomas Lamy (c) 2025\n")
                   wxT("email: ampmeter@macwake.de"),
               wxT("MacWake USB-C Amp Meter OSD"), wxOK | wxICON_INFORMATION,
               this);
}
