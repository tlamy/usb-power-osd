...
wxBEGIN_EVENT_TABLE(DeviceSelectionDialog, wxDialog)
    EVT_BUTTON(ID_REFRESH_BUTTON, DeviceSelectionDialog::OnRefreshSerial)
    EVT_BUTTON(wxID_OK, DeviceSelectionDialog::OnOK)
    EVT_BUTTON(wxID_CANCEL, DeviceSelectionDialog::OnCancel)
    EVT_CLOSE(DeviceSelectionDialog::OnClose)
    EVT_LIST_ITEM_SELECTED(ID_DEVICE_LIST, DeviceSelectionDialog::OnSerialItemSelected)
    EVT_LIST_ITEM_ACTIVATED(ID_DEVICE_LIST, DeviceSelectionDialog::OnSerialItemActivated)
    EVT_TIMER(ID_SCAN_TIMER, DeviceSelectionDialog::OnScanTimer)
wxEND_EVENT_TABLE()

DeviceSelectionDialog::DeviceSelectionDialog(wxWindow *parent)
    : wxDialog(parent, wxID_ANY, "Select Communication Device", wxDefaultPosition, wxSize(500, 400))
      , m_deviceSelected(false)
      , m_scanTimeRemaining(0) {
...
void DeviceSelectionDialog::CreateControls() {
    ...
    serialSizer->Add(new wxStaticText(m_serialPanel, wxID_ANY, "Available Serial Ports:"),
                     0, wxALL, 5);

    m_serialList = new wxListCtrl(m_serialPanel, ID_DEVICE_LIST, wxDefaultPosition, wxDefaultSize,
    ...
    m_refreshSerialButton = new wxButton(m_serialPanel, ID_REFRESH_BUTTON, "Refresh");
    ...
    try {
        ...
        bleAvailable = true; // If we can create it without exception, assume it's available
    } catch (...) {
        bleAvailable = false;
    }
    ...
    if (bleAvailable) {
        ...
        bleSizer->Add(new wxStaticText(m_blePanel, wxID_ANY, "Bluetooth LE Devices:"),
                      0, wxALL, 5);

        m_bleList = new wxListCtrl(m_blePanel, ID_DEVICE_LIST + 1, wxDefaultPosition, wxDefaultSize,
                                   wxLC_REPORT | wxLC_SINGLE_SEL);
        ...
        m_scanBleButton = new wxButton(m_blePanel, ID_REFRESH_BUTTON + 1, "Scan for Devices");
        ...
        // Bind BLE events
        m_scanBleButton->Bind(wxEVT_COMMAND_BUTTON_CLICKED, &DeviceSelectionDialog::OnScanBLE, this);
        m_bleList->Bind(wxEVT_LIST_ITEM_SELECTED, &DeviceSelectionDialog::OnBLEItemSelected, this);
        m_bleList->Bind(wxEVT_LIST_ITEM_ACTIVATED, &DeviceSelectionDialog::OnBLEItemActivated, this);
    } else {
...
void DeviceSelectionDialog::OnScanTimer(wxTimerEvent & WXUNUSED(event)) {
    ...
    if (m_scanTimeRemaining <= 0) {
        ...
        if (m_scanStatus) {
            m_scanStatus->SetLabel(wxString::Format("Scanning... %d seconds remaining", m_scanTimeRemaining));
        }
...
void DeviceSelectionDialog::OnBLEItemSelected(wxListEvent &event) {
    ...
    if (index >= 0 && index < static_cast<long>(m_bleDevices.size())) {
        ...
        m_selectedDevice.displayName = device.name.IsEmpty() ? device.address : device.name;
        m_deviceSelected = true;
        m_okButton->Enable(true);
    }
}

void DeviceSelectionDialog::OnSerialItemActivated(wxListEvent & WXUNUSED(event)) {
    if (m_deviceSelected) {
        EndModal(wxID_OK);
    }
}
...
void DeviceSelectionDialog::StartBLEScan() {
    ...
    if (m_bleEnumerator.StartScan(5000)) {
        ...
        wxMessageBox("Failed to start Bluetooth scan. Please check if Bluetooth is enabled.",
...
void DeviceSelectionDialog::UpdateBLEList() {
    ...
    for (size_t i = 0; i < m_bleDevices.size(); ++i) {
        ...
        long itemIndex = m_bleList->InsertItem(i, device.name.IsEmpty() ? "Unknown Device" : device.name);
