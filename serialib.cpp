...
/*!
  ...
  */
int serialib::writeChar(const char Byte) const {
#if defined(_WIN32) || defined(_WIN64)
...
/*!
   ...
   by '\0')
     ...
     \return    -1 error while writting data
  */
int serialib::writeString(const char *receivedString) const {
#if defined(_WIN32) || defined(_WIN64)
  // Number of bytes written
  DWORD dwBytesWritten;
  // Write the string
...
/*!
     ...
     \return -1 error while writting data
  */
int serialib::writeBytes(const void *Buffer, const unsigned int NbBytes) const {
#if defined(_WIN32) || defined(_WIN64)
  // Number of bytes written
  DWORD dwBytesWritten;
  // Write data
...
#if defined(__linux__) || defined(__APPLE__)
  // Write data
  if (write(fd, Buffer, NbBytes) != static_cast<ssize_t>(NbBytes))
    return -1;
  // Write operation successfull
  return 1;
#endif
...
/*!
  ...
  */
int serialib::readChar(char *pByte, unsigned int timeOut_ms) const {
#if defined(_WIN32) || defined(_WIN64)
...
int serialib::readBytes(void *buffer, unsigned int maxNbBytes,
                        ...
                        unsigned int sleepDuration_us) const {
#if defined(_WIN32) || defined(_WIN64)
...
unsigned long int timeOut::elapsedTime_ms() const {
#if defined(NO_POSIX_TIME)
  // Current time
